---
title: 02 | Cantilever Beams / Load Types
sidebar_position: 3
week: week-2
date: 2026-07-31
topic: workshop
youtube_id: Z1fp84hK2FQ
slides_url: https://docs.google.com/presentation/d/1VrIZUqOFp-8XG2ZVUueSoOdMf3ovRRBcZli8cCLe354/edit?usp=sharing
related_docs:
  - Intro | Karamba Foundations Notes
downloads:
  - label: Grasshopper File
    file: https://res.cloudinary.com/dbf3aqsxu/raw/upload/v1785481907/Cantiliver_ymwjcv.gh
  - label: Rhino File
    file: https://res.cloudinary.com/dbf3aqsxu/raw/upload/v1785481907/cantilever_z0n6k4.3dm
---
This session covers using a few new components to see the numbers behind three different types of load cases.
